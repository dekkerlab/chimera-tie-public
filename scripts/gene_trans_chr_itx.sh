...
grep "${gene}" ${itx_file} | awk "((\$19==\"${gene}\") && (\$23!=\"${gene}\"))||((\$19!=\"${gene}\") && (\$23==\"${gene}\")){print \$0}" > ${gene}_trans.itx
echo " lines per file"
echo "----------------"
wc -l ${gene}_trans.itx
awk "(\$23==\"${gene}\"){print \$12}" ${gene}_trans.itx > loc1_${gene}
wc -l loc1_${gene}
awk "(\$19==\"${gene}\"){print \$4}" ${gene}_trans.itx > loc2_${gene}
wc -l loc2_${gene}
cat loc2_${gene} loc1_${gene} > loc12_${gene}
wc -l loc12_${gene}
sort -n loc12_${gene} | uniq -c > loc_${gene}_count
wc -l loc_${gene}_count
awk "{print \$1 \"\t\" \$2}" loc_${gene}_count > loc_${gene}_count.tsv
wc -l loc_${gene}_count.tsv
